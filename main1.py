...
for result in detection_results:
    ...
    for box in boxes:
        ...
        # بررسی عبور از خط مرجع
        if center_x_roi >= line_x_global - roi_x:  # تبدیل line_x به مختصات ROI
            ...
            pose_results = pose_model(billet_roi, verbose=False)
            ...
            if len(pose_results) > 0 and len(pose_results[0].keypoints.xy) > 0:
                ...
                print(f"   ❌ No keypoints found by pose model! (Check model input format)")
...
print(f"✅ Billets outside ROI: {total_boxes - (billet_id - 1)}")
print(f"✅ Billets outside Line: {total_boxes - (billet_id - 1)}")
print(f"⚠️ Note: Billets outside ROI or Line are ignored.")
