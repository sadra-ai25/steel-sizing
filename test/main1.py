...
def main():
    ...
    try:
        ...
        # We only need the pose model as it handles detection + keypoints
        ...
        # ======================
        # 5. POSE INFERENCE (Detection + Keypoints)
        # ======================
        logger.info("Running pose model on FULL FRAME...")
        img_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        results = pose_model(img_rgb, verbose=False)
        ...
        if r.boxes is not None:
            ...
            logger.info(f"Pose model detected {total_detections} objects.")
            
            # Iterate over detections provided by the POSE MODEL
            for i in range(len(r.boxes)):
                ...
                # Get bounding box from Pose Model
                ...
                if r.keypoints is not None:
                    # Get keypoints for THIS specific detection (index i)
                    raw_keypoints = r.keypoints.data[i].cpu().numpy()
                    
                    # --- FIX: Handle shape (1, N, 2) -> (N, 2) using squeeze ---
                    ...
                    if keypoints.shape[0] >= 2:
                        ...
                        logger.info(f"Billet {billet_id} detected with {num_keypoints} keypoints")
                        
                        # Log all keypoints (Safe iteration)
                        for idx in range(num_keypoints):
                            x = keypoints[idx][0]
                            y = keypoints[idx][1]
                            logger.debug(f"Keypoint {idx}: x={x:.2f}, y={y:.2f}")
                        
                        # --- FIX: Exact logic from the working snippet ---
                        ...
                        logger.info(f"  Total Length (Point 0 to 1): {distance_px:.2f} px -> {length_mm if length_mm else 'N/A'} mm")
                        ...
                        # Calculate and log all segment distances (Optional, kept from original)
                        for k in range(num_keypoints):
                            for l in range(k + 1, num_keypoints):
                                pt_a = keypoints[k]
                                pt_b = keypoints[l]
                                dist = np.sqrt((pt_b[0] - pt_a[0])**2 + (pt_b[1] - pt_a[1])**2)
                                logger.info(f"  Distance Point {k} to {l}: {dist:.2f} px")
                    else:
            ...
            logger.warning("No objects detected by pose model.")
